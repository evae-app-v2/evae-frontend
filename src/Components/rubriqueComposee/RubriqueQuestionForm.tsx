...
import { RubriqueQuestionService } from "../../services/RubriqueQuestionService"; // Import the QuestionService
...
//   const [questions, setQuestions] = useState([]); // State to store questions
  ...
  const [questions, setQuestions] = useState<Question[]>([]); // Explicitly specify the type
  ...
  useEffect(() => {
    if (initialData) {
      ...
      // Fetch and set questions when the component mounts or when initialData changes
      loadQuestions(initialData.id); // Assuming id is the rubriqueId
    }
  }, [initialData]);
  ...
  const handleClose = () => {
    setOpen((prevOpen) => !prevOpen);
    setDesignation("");
    setQuestions([]); // Clear questions when closing the form
  };

  const loadQuestions = async (rubriqueId: any) => {
    try {
      const questions = (await new RubriqueQuestionService().getQuestionsByRubrique(rubriqueId));
    ...
    return (
      <ul>
        {questions.map((question) => (
          <li key={question.id}>{question.intitule}</li>
        ))}
      </ul>
    );
  };
      ...
      >
        <Card className="mx-auto w-full max-w-[24rem]" placeholder={undefined}>
          <CardBody className="flex flex-col gap-4" placeholder={undefined}>
            ...
            {renderQuestions()} {/* Render the list of questions */}
